...
class MT_3_ResNet:
    ...
    def res_block(self, net, filters=64, kernel=3, name='resnet_convX', reuse=False):
        ...
        net = layers.conv2d(net, filters=filters, kernel_size=kernel, stride=1, padding='same', name=name + '_2', reuse=reuse,
                            activation_fn=None, weights_regularizer=layers.l2_regularizer(REGULARIZER_SCALE),
                            normalizer_fn=layers.batchNormalization, normalizer_params=bn_params)
    ...
    def res_block_first(self, net, filters=64, kernel=3, stride=1, reuse=False, name='resnet_convX'):
        ...
        tmp_logits = layers.conv2d(net, filters=filters, kernel_size=kernel, stride=stride, padding='same', name=name + '_shortcut',
                                   reuse=reuse, weights_regularizer=layers.l2_regularizer(REGULARIZER_SCALE), activation_fn=None,
                                   normalizer_fn=layers.batchNormalization, normalizer_params=bn_params)
